	...
	requestAjax : function(param, returnAjaxCallBack){
		$.ajax({
	        ...
	        error : function(request,status,error){
        		...
        		/*if(UT.isEqual(resultData.resultValue, "Y")){
					
					if(UT.isFunction(returnAjaxCallBack)) returnAjaxCallBack(resultData);
					UT.showAlert(DEF.messages.saveComplete);
				}*/
		...
		Util.requestHide(requestUrl, param, function(resultData){
			...
			if(Util.isObject(menuInfo)){
				...
				if( !Util.isEmpty(menuLinkUrl)){
					Util.requestLoad(menuLinkUrl+"?menuCode="+param.menuInfo.menuCode, param.menuInfo, function(resultData){
						//인자로 함수 이름 넣어줍니다.
						try{
							
							var jsonResult = JSON.parse(resultData);
								
							//if( jsonResult.resultMsg != null ) alert(jsonResult.resultMsg);
